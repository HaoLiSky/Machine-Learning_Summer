"""
This module provides an argument parser for the framework functionality.
...
import numpy as np
from io_structures import read_parameters, parse_property, structures_to_hdf5
    ...
    with h5py.File(filename, 'r', libver='latest') as h5f:
        ...
        try:
            ...
            fingerprint_keys = sorted((set((p_dset.keys()))
                                      .difference({'system', 'coordinates'})))
            p_dims = [p_dset[key][()].shape[1] for key in fingerprint_keys]
        except KeyError:
            p_dims = []
            ...
            if 'coordinates' in s_dset[s_name].keys():
                ...
                assertions = [data['natoms'] == sum(data['species_counts']),
                              ...
                              (len(data['species_counts'])
                               == len(sys_elements)),
                              np.asarray(data['unit']).shape == (3, 3)]
